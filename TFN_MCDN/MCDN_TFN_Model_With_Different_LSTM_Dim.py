...
class LSTHMCell(nn.Module):
    ...
    def init_weights(self):
        ...
        # self.linear.bias = nn.init.xavier_normal(self.linear.bias)
...
class MCDN(nn.Module):
    ...
    def __init__(self, input_dim, lstm_dim, fusion_dim, nattentions, dropout_lstm=0.5):
        ...
        for i in range(self.modality):
            ...
            self.LSTHMCells.append(LSTHMCell(self.input_dim[i], self.lstm_dim[i], self.fusion_dim))
        ...
        for i in range(nattentions):
            # self.attentions.append(nn.Linear((self.lstm_dim+1)**self.modality, (self.lstm_dim+1)**self.modality))
            self.attentions.append(nn.Linear((self.lstm_dim[0]+1)*(self.lstm_dim[1]+1)*(self.lstm_dim[2]+1), (self.lstm_dim[0]+1)*(self.lstm_dim[1]+1)*(self.lstm_dim[2]+1)))

        # Define the final weights of the network computes Z_t
        # self.Ws = nn.Linear((self.lstm_dim+1)**self.modality * self.nattentions, (self.lstm_dim+1)**self.modality)
        # self.Wz = nn.Linear((self.lstm_dim+1)**self.modality, self.fusion_dim)
        self.local_global_network = nn.Linear((self.lstm_dim[0]+1)*(self.lstm_dim[1]+1)*(self.lstm_dim[2]+1),(self.lstm_dim[0]+1)*(self.lstm_dim[1]+1)*(self.lstm_dim[2]+1))
        self.Ws = nn.Linear((self.lstm_dim[0]+1)*(self.lstm_dim[1]+1)*(self.lstm_dim[2]+1) * self.nattentions, (self.lstm_dim[0]+1)*(self.lstm_dim[1]+1)*(self.lstm_dim[2]+1))
        self.Wz = nn.Linear((self.lstm_dim[0]+1)*(self.lstm_dim[1]+1)*(self.lstm_dim[2]+1), self.fusion_dim)
    ...
    def init_weights(self):
        ...
        # self.Ws.weight = nn.init.xavier_normal(self.Ws.weight)
        # self.Ws.bias = nn.init.xavier_normal(self.Ws.bias)

        # self.Wz.weight = nn.init.xavier_normal(self.Wz.weight)
        # self.Wz.bias = nn.init.xavier_normal(self.Wz.bias)

        self.Ws.weight = nn.init.normal(self.Ws.weight)
        self.Ws.bias = nn.init.normal(self.Ws.bias)

        self.Wz.weight = nn.init.normal(self.Wz.weight)
        self.Wz.bias = nn.init.normal(self.Wz.bias)
    ...
    def forward(self, input_seq):
        ...
        for i in range(self.modality):
            hidden.append(Variable(torch.zeros(1, self.lstm_dim[i])))
            cell_state.append(Variable(torch.zeros(1, self.lstm_dim[i])))
        ...
        for time_step in input_seq:
            ...
            cubic_hidden_states = Tensor_Fusion_Network(hidden[0],hidden[1],hidden[2])
            hidden_all = cubic_hidden_states.compute_cross_product()
            # hidden_all = hidden_all.view(-1,2)
            # hidden_all = torch.cat(hidden, dim=1)
            hidden_all = torch.unsqueeze(hidden_all,0)
            ...
            for attention in attended_all:
                reshaped_attended.append(self.local_global_network(attention))

            reshaped_all = reshaped_attended[0]
            for i in range(1,len(reshaped_attended)):
                reshaped_all = torch.cat((reshaped_all,reshaped_attended[i]))
            
            # reshaped_all = torch.cat(reshaped_attended, dim=1)
            reshaped_all = reshaped_all.view(1,-1)
            # print reshaped_all.size()
            # s = raw_input()
